...
func (d *DrainRequestor) DrainNode(ctx context.Context, node *corev1.Node,
	...
	d.log.WithName("drainNode")
	d.log.Info("Node drain requested")

	completed, err := d.platformHelpers.OpenshiftBeforeDrainNode(ctx, node)
	if err != nil {
		d.log.Error(err, "error running OpenshiftDrainNode")
		return false, err
	}

	if !completed {
		d.log.Info("OpenshiftDrainNode did not finish re queue the node request")
		return false, nil
	}
	...
	if err != nil {
		d.log.Error(err, "error creating node maintenance")
		return false, err
	}
	...
	if cond != nil {
		if cond.Reason == maintenancev1alpha1.ConditionReasonReady {
			...
			d.log.Info("drainNode(): Drain completed")
			return true, nil
		}
	}
...
func (d *DrainRequestor) CompleteDrainNode(ctx context.Context, node *corev1.Node) (bool, error) {
	...
	if err != nil {
		d.log.Error(err, "failed to complete openshift draining")
		return false, err
	}
	d.log.V(consts.LogLevelDebug).Info("CompleteDrainNode:()", " nodeMaintenance deleted",
...
// GetRequestorOptsFromEnvs returns requstor upgrade related options according to
